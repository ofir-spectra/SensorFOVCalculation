...
# 1. Compute max IFOV per pixel (corners only), with on-disk caching:
@njit(parallel=True, fastmath=True, cache=True)
def compute_max_projected_pixel_size_jit(hw, hh, px, py, cam_pos, x_cam, y_cam, z_cam, H):
    x_edges = np.linspace(-hw, hw, px + 1)
    y_edges = np.linspace(-hh, hh, py + 1)
    max_size = 0.0
    corners = [(0,0),(px-1,0),(px-1,py-1),(0,py-1)]
    for (i,j) in corners:
        pts = [(x_edges[i], y_edges[j]),
               (x_edges[i+1], y_edges[j]),
               (x_edges[i+1], y_edges[j+1]),
               (x_edges[i], y_edges[j+1])]
        world = np.empty((4,2))
        for k, (xi, yi) in enumerate(pts):
            ray = xi*x_cam + yi*y_cam + H*z_cam
            rn = np.linalg.norm(ray)
            if rn<1e-8 or ray[2]==0: continue
            ray /= rn
            t = -cam_pos[2]/ray[2]
            p = cam_pos + t*ray
            world[k] = p[:2]
        for k in range(4):
            d = np.hypot(*(world[k] - world[(k+1)%4]))
            if d>max_size: max_size = d
    return max_size

@njit(parallel=True, fastmath=True)
def build_ifov_map_jit(px, py, hw, hh, cam_pos, x_cam, y_cam, z_cam, H):
    x_edges = np.linspace(-hw, hw, px+1)
    y_edges = np.linspace(-hh, hh, py+1)
    ...
    for j in prange(py):
        for i in range(px):
            ...
            for corner in ((i,j),(i+1,j),(i+1,j+1),(i,j+1)):
                xi = x_edges[corner[0]]; yi = y_edges[corner[1]]
                ray = xi*x_cam + yi*y_cam + H*z_cam
                rn = np.linalg.norm(ray)
                if rn < 1e-8 or ray[2]==0: continue
                ray = ray / rn
                ...
                p = cam_pos + t*ray
                # next corner
                ni, nj = corner[(0,1)][0]+(1 if corner[0]<i+1 else -1), corner[1]
...
def clamp_pixels(px, py, max_px):
    was_clamped = px > max_px or py > max_px
    return min(px, max_px), min(py, max_px), was_clamped
...
def calculate_water_coverage_for_angle(params):
    ...
    overlay_xx, overlay_yy = np.meshgrid(sensor_x, sensor_y)
    ...
    for i in range(coverage_grid_size):
        for j in range(coverage_grid_size):
            pixel_x = overlay_xx[i, j]
            pixel_y = overlay_yy[i, j]
...
def calculate_target_resolution(required_half_width, required_half_height, resolution, cam_pos, x_cam, y_cam, z_cam, H, enforce_ifov=False):
    """
    Calculate the realistic target resolution without aspect ratio constraints.
    This provides the minimum pixels required per axis to meet the projected IFOV everywhere.
    """
    # Calculate minimum sensor dimensions needed (no aspect ratio constraint)
    target_sensor_width_mm = 2 * required_half_width
    target_sensor_height_mm = 2 * required_half_height
    
    # Calculate basic pixel requirements
    ...
    if enforce_ifov:
        # When IFOV is enforced, we need to ensure the projected IFOV everywhere is <= required resolution
        # This may require more pixels than the basic calculation
        target_sensor_half_width = target_sensor_width_mm / 2
        target_sensor_half_height = target_sensor_height_mm / 2
        
        # Start with basic pixel count and iterate until IFOV requirement is met
        max_iterations = 100
        iteration = 0
        
        while iteration < max_iterations:
            # Calculate maximum projected IFOV with current pixel count
            max_ifov = compute_max_projected_pixel_size_jit(
                target_sensor_half_width, target_sensor_half_height, 
                pixels_x_target, pixels_y_target,
                cam_pos, x_cam, y_cam, z_cam, H
            )
            
            # If IFOV requirement is met, we're done
            if max_ifov <= resolution * 1.01:  # Small tolerance
                break
            
            # Otherwise, increase pixel count proportionally
            ifov_factor = max_ifov / resolution
            pixels_x_target = int(np.ceil(pixels_x_target * ifov_factor))
            pixels_y_target = int(np.ceil(pixels_y_target * ifov_factor))
            
            iteration += 1
        
        # Update sensor dimensions based on final pixel count
        target_sensor_width_mm = pixels_x_target * resolution
        target_sensor_height_mm = pixels_y_target * resolution
    
    return pixels_x_target, pixels_y_target, target_sensor_width_mm, target_sensor_height_mm
...
def calculate_projection(params, x_range_mm=(-350, 350), y_range_mm=(-350, 350), n_ticks=9):
    ...
    # Sensor FOV calculation (just like elsewhere in your code)
    # These determine the physical size needed to cover the water area with margin:
    sensor_half_width = W / 2
    sensor_half_height = L / 2

    MAX_OVERLAY_GRID = 50
    overlay_grid_N = min(int(overlay_grid_N), MAX_OVERLAY_GRID)
    overlay_x = np.linspace(-sensor_half_width, sensor_half_width, overlay_grid_N)
    overlay_y = np.linspace(-sensor_half_height, sensor_half_height, overlay_grid_N)
    overlay_xx, overlay_yy = np.meshgrid(overlay_x, overlay_y)
    ...
    pixels_x_sensor = min(pixels_x_sensor, 200)
    pixels_y_sensor = min(pixels_y_sensor, 200)
    pixels_x_target = min(pixels_x_target, 200)
    pixels_y_target = min(pixels_y_target, 200)
    ...
    max_pixels = int(params.get('MaxSensorRes', 5000))
    pixels_x_sensor, pixels_y_sensor, was_clamped = clamp_pixels(
        pixels_x_sensor, pixels_y_sensor, max_pixels
    )
    warning = None
    if was_clamped and params.get('EnforceIFOV', False):
        warning = "Max Sensor Pixels limit reached—IFOV requirement not met."
...
def get_plot_data(params, overlay_grid_N=50, x_range_mm=(-350, 350), y_range_mm=(-350, 350), n_ticks=9, smoothness=2, smoothing_window=1):


    A = params['A']              # Camera height [mm]
    B = params['B']              # Water spot length [mm]
    C = params['C']              # Water spot width [mm]
    margin_percent = params['Margin']
    try:
        margin_percent = float(margin_percent)
    except Exception:
        margin_percent = 0.0
    margin_factor = 1.0 + (margin_percent / 100.0)
    L = B * margin_factor        # Water spot length (with margin)
    W = C * margin_factor        # Water spot width (with margin)
    sensor_half_width = W / 2    # Half width of FOV on sensor
    sensor_half_height = L / 2   # Half height of FOV on sensor
        # Limit overlay grid to 50 for smooth performance
    MAX_OVERLAY_GRID = 50
    
    overlay_grid_N = min(int(overlay_grid_N), MAX_OVERLAY_GRID)

    # Calculate sensor half width/height as before
    #sensor_half_width = sensor_half_width  # already calculated above
    #sensor_half_height = sensor_half_height  # already calculated above

    # Overlay grid, now created ONCE, at top-level scope
    overlay_x = np.linspace(-sensor_half_width, sensor_half_width, overlay_grid_N)
    overlay_y = np.linspace(-sensor_half_height, sensor_half_height, overlay_grid_N)
    overlay_xx, overlay_yy = np.meshgrid(overlay_x, overlay_y)
    ...
    # Always start with geometry-based pixel counts
    pixels_x_geom = max(2, int(np.ceil(sensor_width_mm / resolution)))
    pixels_y_geom = max(2, int(np.ceil(sensor_height_mm / resolution)))

    if enforce_ifov:
        max_ifov = compute_max_projected_pixel_size_jit(
            sensor_half_width, sensor_half_height,
            pixels_x_geom, pixels_y_geom,
            cam_pos, x_cam, y_cam, z_cam, H
        )
        upscale = max(1.0, max_ifov / resolution)
        pixels_x_req = int(np.ceil(pixels_x_geom * upscale))
        pixels_y_req = int(np.ceil(pixels_y_geom * upscale))
    else:
        max_ifov = resolution
        pixels_x_req = pixels_x_geom
        pixels_y_req = pixels_y_geom

    # Always clamp to max_pixels input, regardless of enforcement
    pixels_x_sensor, pixels_y_sensor, was_clamped = clamp_pixels(
        pixels_x_req, pixels_y_req, max_pixels
    )
    if was_clamped:
        factor = max(pixels_x_req / max(pixels_x_sensor, 1),
                    pixels_y_req / max(pixels_y_sensor, 1))
        max_ifov = resolution * factor
    # Estimate minimum IFOV at image center
    centre_half_w = sensor_half_width / max(pixels_x_sensor, 1)
    centre_half_h = sensor_half_height / max(pixels_y_sensor, 1)
    min_ifov = 2 * np.sqrt(centre_half_w**2 + centre_half_h**2) / 2
    ...
    # REALISTIC TARGET RESOLUTION: Calculate minimum pixels without aspect ratio constraints
    pixels_x_target, pixels_y_target, target_sensor_width_mm, target_sensor_height_mm = calculate_target_resolution(
    required_half_width, required_half_height, resolution, cam_pos, x_cam, y_cam, z_cam, H, enforce_ifov)   
    ...
    def calculate_water_coverage_efficiency():
        coverage_grid_size = overlay_grid_N  # uses the same N as grid
        water_pixels = 0
        total_pixels = coverage_grid_size * coverage_grid_size
        for i in range(coverage_grid_size):
            for j in range(coverage_grid_size):
    ...
    if params.get("EnforceIFOV", False):
        ...
        # --- Compute per-row IFOV map over the full sensor grid ---
    # px = number of columns, py = number of rows
    px = int(np.ceil(2 * sensor_half_width / resolution))
    py = int(np.ceil(2 * sensor_half_height / resolution))
    max_pixels = params.get('MaxSensorRes', px)
    px = min(px, max_pixels)
    py = min(py, max_pixels)
    ...
    # Build full IFOV map [px × py]
    ifov_map = np.zeros((px, py))
    for j in range(py):
        for i in range(px):
            # corners of pixel (i,j)
            corners = [
                (x_edges[i],   y_edges[j]),
                (x_edges[i+1], y_edges[j]),
                (x_edges[i+1], y_edges[j+1]),
            ...
            for (xs, ys) in corners:
                ray = xs*x_cam + ys*y_cam + H*z_cam
                ray = ray / np.linalg.norm(ray)
                t = -cam_pos[2] / ray[2]
                pt = cam_pos + t*ray
                world_pts.append(pt[:2])
            # longest side length = IFOV
            lengths = [np.linalg.norm(world_pts[k] - world_pts[(k+1)%4]) for k in range(4)]
            ifov_map[i, j] = max(lengths)

    # Pass this map back in results:
    data_ifov_map = ifov_map

    # Enforce max-pixels cap on realistic sensor
    max_pixels = int(params.get('MaxSensorRes', 5000))
    # Compute unclamped sensor pixel counts
    pixels_x_req = int(np.ceil(sensor_width_mm / resolution))
    pixels_y_req = int(np.ceil(sensor_height_mm / resolution))
    # Apply clamp
    pixels_x_sensor, pixels_y_sensor, was_clamped = clamp_pixels(
        pixels_x_req, pixels_y_req, max_pixels)
    # Prepare warning if cap prevented full IFOV enforcement
    warning = None
    if was_clamped and params.get('EnforceIFOV', False):
        warning = "Max Sensor Pixels limit reached—IFOV requirement not met."
    ...
    return {
    # Basic parameters
    'A': A, 'B': B, 'C': C, 'H': H, 'L': L, 'W': W, 'Resolution': resolution,
    ...
    'overlay_xx': overlay_xx,
    'overlay_yy': overlay_yy,
}
