...
class ProjectionApp:
    ...
    def plot_projection(self):
        ...
        # Resolution per tile (for a 1x1 setup)
        pixels_x_per_tile = plot_data.get("pixels_x_sensor", 0)
        pixels_y_per_tile = plot_data.get("pixels_y_sensor", 0)

        # Total resolution for the full sensor (including dead zones)
        total_pixels_x = nx * pixels_x_per_tile + (nx - 1) * deadzone_px  # Horizontal tiles + dead zones
        total_pixels_y = ny * pixels_y_per_tile + (ny - 1) * deadzone_px  # Vertical tiles + dead zones

        # Check if total resolution exceeds maximum allowed pixels
        max_res = params.get("MaxSensorRes", 5000)
        scaling_factor_x = 1.0
        scaling_factor_y = 1.0
        if total_pixels_x > max_res:
            scaling_factor_x = max_res / total_pixels_x
            total_pixels_x = max_res
        if total_pixels_y > max_res:
            scaling_factor_y = max_res / total_pixels_y
            total_pixels_y = max_res

        # Apply scaling factors to resolution per tile
        pixels_x_per_tile = int(pixels_x_per_tile * scaling_factor_x)
        pixels_y_per_tile = int(pixels_y_per_tile * scaling_factor_y)

        # Recalculate IFOV based on updated resolution
        ifov_x = params.get("Resolution", 0.22) / scaling_factor_x
        ifov_y = params.get("Resolution", 0.22) / scaling_factor_y

        # Update the plot data with the total resolution and recalculated IFOV
        ...
        plot_data['ifov_x'] = ifov_x
        plot_data['ifov_y'] = ifov_y
        ...
        print(f"IFOV: {ifov_x:.4f} mm/px (X-axis), {ifov_y:.4f} mm/px (Y-axis)")
