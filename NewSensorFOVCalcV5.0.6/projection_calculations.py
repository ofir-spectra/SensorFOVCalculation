...
def get_plot_data(params, x_range_mm=(-350, 350), y_range_mm=(-350, 350), n_ticks=9, smoothness=2, smoothing_window=1):
    ...
    # ADD THESE LINES after ifov_map is filled (just before the return statement):
    min_ifov = float(np.nanmin(ifov_map)) if ifov_map is not None else 0.0
    max_ifov = float(np.nanmax(ifov_map)) if ifov_map is not None else 0.0
    ...
    return {
        ...
        'ifov_map': ifov_map,     # ADDED
        'min_ifov': min_ifov,
        'max_ifov': max_ifov,

        'px_sensor': px,          # ADDED for indexing
        'py_sensor': py           # ADDED for indexing
    }
