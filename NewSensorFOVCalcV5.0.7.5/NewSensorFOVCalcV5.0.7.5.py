...
class ProjectionApp:
    ...
    def plot_projected_view(self, data):
        ...
        if ifov_map is not None and self.ifov_enforce_var.get():
            ...
            shift_axis = data.get('shift_axis', 'X')

            if shift_axis == 'X':
                # For each column: if any dot IN the trapezoid has bad IFOV, paint ALL in-trapezoid dots in that column red
                for i in range(px):
                    col_idxs = [i * py + j for j in range(py)]
                    # Only consider in_trapezoid pixels for this column
                    any_bad = any(
                        in_trapezoid[idx] and ifov_map[i, j] > req_res
                        for j, idx in enumerate(col_idxs)
                    )
                    for j, idx in enumerate(col_idxs):
                        if idx >= N or not in_trapezoid[idx]:
                            continue
                        color_arr[idx] = (0.8, 0.2, 0.2) if any_bad else (0.2, 0.4, 1.0)
            elif shift_axis == 'Y':
                # For each row: if any dot IN the trapezoid has bad IFOV, paint ALL in-trapezoid dots in that row red
                for j in range(py):
                    row_idxs = [i * py + j for i in range(px)]
                    any_bad = any(
                        in_trapezoid[idx] and ifov_map[i, j] > req_res
