...
class ProjectionApp:
    def __init__(self, root):
        self.root = root
        self.root.title(f"NewSensorFOVCalc{APP_VERSION}")
        self.root.geometry("1900x1000")
        
        title_label = tk.Label(root, text=f"NewSensorFOVCalc{APP_VERSION}", font=("Arial", 20, "bold"))
    ...
    def find_image_case_insensitive(filename):
        """Find image file in current directory, case-insensitive, for .png or .jpg"""
        folder = os.path.dirname(resource_path(filename))
    ...
    def plot_projected_view(self, data):
        ...
        overlay_text = f"""REALISTIC SENSOR ({data['aspect_ratio_used']}): {data['pixels_x_sensor']} x {data['pixels_y_sensor']} [px]
NAIVE RESOLUTION: {data['pixels_x_naive']} x {data['pixels_y_naive']} [px]
WATER COVERAGE: {data['water_coverage_percent']:.1f}%
OPTIMAL TILT ANGLE: {data['optimal_angle']:.1f}°
PROJECTION OFFSET: {data['projection_offset']:.1f} mm from optical axis"""
