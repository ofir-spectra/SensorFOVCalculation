...
class ProjectionApp:
    def __init__(self, root):
        self.root = root
        self.root.title("SENSOR SIMULATION V5.0.8")

        # Make window maximized (full screen)
        try:
            self.root.state('zoomed')  # For Windows
        except:
            self.root.attributes('-zoomed', True)  # For Mac/Linux

        self.root.minsize(1280, 720)  # Minimum window size

        # Let the window use most of the screen
        for i in range(4):
            self.root.grid_rowconfigure(i, weight=1)
        for i in range(2):
            self.root.grid_columnconfigure(i, weight=1)

        # The rest of your normal setup code here...
    ...
    def update_simulation_results(self, data):
        ...
        for param, value, unit in results:
            ...
            # -------  SHOW WARNING IF PIXEL LIMIT HITS  -------
        if data.get('warning'):
            if not hasattr(self, "ifov_warning_label"):
                self.ifov_warning_label = tk.Label(self.results_frame, text="", fg='red', font=self.param_font)
                self.ifov_warning_label.pack(anchor='w', pady=(6,0))
            self.ifov_warning_label.config(text=data['warning'])
        elif hasattr(self, "ifov_warning_label"):
            self.ifov_warning_label.config(text="")
    ...
    def setup_params_frame(self):
        ...
        self.overlay_grid_var = tk.IntVar(value=50)  # Default grid size: 50
        overlay_lbl = tk.Label(param_frame, text="Overlay Grid N×N:", font=self.param_font)
        overlay_lbl.grid(row=next_row+9, column=0, sticky="e", padx=5, pady=5)
        overlay_entry = tk.Entry(param_frame, textvariable=self.overlay_grid_var, font=self.param_font, width=6)
        overlay_entry.grid(row=next_row+9, column=1, padx=5, pady=5, sticky="w")
    ...
    def plot_projection(self):
        ...
        overlay_grid_N = self.overlay_grid_var.get()
        plot_data = get_plot_data(params, overlay_grid_N=overlay_grid_N,smoothness=params.get('Smoothness', 2),
    ...
    def plot_projected_view(self, data):
        ...
        color_arr = np.full((N, 3), (0.2, 0.4, 1.0))  # blue
            ...
            # Color rows based on IFOV vs required resolution
        req_res = data['Resolution']
        px, py = data['px_sensor'], data['py_sensor']  # sensor dims after clamping
        ifov_map = data['ifov_map']
        from matplotlib.path import Path
        trap = Path(data['proj_rect_outline'])
        x0, x1 = min(data['box_x_mm']), max(data['box_x_mm'])
        y0, y1 = min(data['box_y_mm']), max(data['box_y_mm'])

        # Only when IFOV enforcement is ON
        if ifov_map is not None and self.ifov_enforce_var.get():
            # Compute max IFOV per row
            row_ifov = np.max(ifov_map, axis=0)  # shape: (py,)
            # Find first row index where IFOV ≤ requirement
            thresh = next((j for j, v in enumerate(row_ifov) if v <= req_res), py)

            # Masks for inside rectangle & trapezoid
            pts = data['proj_rect_pts']
            in_rect = (pts[:,0]>=x0)&(pts[:,0]<=x1)&(pts[:,1]>=y0)&(pts[:,1]<=y1)
            in_trap = trap.contains_points(pts)

            # Color by row
            for j in range(py):
                color = (0.8,0.2,0.2) if j < thresh else (0.2,0.4,1.0)
                for i in range(px):
                    idx = i*py + j
                    if idx < len(pts) and in_rect[idx] and in_trap[idx]:
                        color_arr[idx] = color
