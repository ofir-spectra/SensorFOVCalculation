...
def get_plot_data(params, x_range_mm=(-350, 350), y_range_mm=(-350, 350), n_ticks=9, smoothness=2, smoothing_window=1):
    ...
    # --- Unpack and sanitize parameters ---
    A = float(params.get('A', 133.0))
    B = float(params.get('B', 317.5))
    C = float(params.get('C', 266.7))
    theta_deg = float(params.get('Tilt', 30.0))
    margin_percent = float(params.get('Margin', 10))
    shift = float(params.get('Shift', 0))
    shift_axis = params.get('ShiftAxis', 'X')
    resolution = float(params.get('Resolution', 0.22))
    ...
    if shift_axis == 'X':
        ...
        z_cam = Ry.dot(initial_optical_axis)
    else:
        theta_corrected = -theta
        Rx = np.array([
            [1, 0, 0],
        ...
        z_cam = Rx.dot(initial_optical_axis)
    z_cam = z_cam / np.linalg.norm(z_cam)

    # Camera basis for the sensor image
    up_guess = np.array([1, 0, 0]) if abs(np.dot(z_cam, [0, 1, 0])) > 0.99 else np.array([0, 1, 0])
    ...
    # --- Function for projecting a 3D world point into the sensor image plane ---
    def project_point(p_world):
        v = p_world - cam_pos
        Xc_ = np.dot(v, x_cam)
        Yc_ = np.dot(v, y_cam)
    ...
    # --- Rectangle grid (projected sensor/target area) ---
    rect_res = 100
    x_rect = np.linspace(-W/2, W/2, rect_res)
    y_rect = np.linspace(-L/2, L/2, rect_res)
    xx_rect, yy_rect = np.meshgrid(x_rect, y_rect)
    ...
    ell_res = 200
    angles = np.linspace(0, 2 * np.pi, ell_res)
    ell_x = (C * margin_factor / 2) * np.cos(angles)
    ell_y = (L * margin_factor / 2) * np.sin(angles)
    ellipse_points = np.column_stack([ell_x, ell_y, np.zeros(ell_res)])
    proj_ellipse_pts = np.array([project_point(p) for p in ellipse_points])
    ellipse_x = proj_ellipse_pts[:, 0]
    ellipse_y = proj_ellipse_pts[:, 1]
    ellipse_cx = np.mean(ellipse_x)
    ellipse_cy = np.mean(ellipse_y)
    ellipse_min_x = np.min(ellipse_x)
    ellipse_max_x = np.max(ellipse_x)
    ellipse_min_y = np.min(ellipse_y)
    ellipse_max_y = np.max(ellipse_y)
    ellipse_half_width = (ellipse_max_x - ellipse_min_x) / 2
    ellipse_half_height = (ellipse_max_y - ellipse_min_y) / 2

    # --- Sensor aspect ratio fit: strictly by smallest allowed sensor rectangle ---
    ALLOWED_ASPECT_RATIOS = {"3:2": 1.5, "2:3": 2/3, "4:3": 4/3, "3:4": 0.75}
    def find_aspect_fit(min_x, max_x, min_y, max_y):
        best_area = float("inf"); best_dims = (0, 0); best_aspect = ""
        Wt = max_x - min_x
        Ht = max_y - min_y
        for name, aspect in ALLOWED_ASPECT_RATIOS.items():
            w1 = max(Wt, Ht * aspect); h1 = w1 / aspect
            if w1 * h1 < best_area and w1 >= Wt and h1 >= Ht:
                best_area = w1 * h1; best_dims = (w1 / 2, h1 / 2); best_aspect = name
            h2 = max(Ht, Wt / aspect); w2 = h2 * aspect
            if w2 * h2 < best_area and w2 >= Wt and h2 >= Ht:
                best_area = w2 * h2; best_dims = (w2 / 2, h2 / 2); best_aspect = name
        return best_dims[0], best_dims[1], best_aspect
    sens_half_w, sens_half_h, aspect_used = find_aspect_fit(ellipse_min_x, ellipse_max_x, ellipse_min_y, ellipse_max_y)
    aspect_ratio_used = aspect_used
    sensor_width_mm = sens_half_w * 2
    sensor_height_mm = sens_half_h * 2
    box_x_mm = np.array([-sens_half_w, sens_half_w, sens_half_w, -sens_half_w, -sens_half_w])
    box_y_mm = np.array([-sens_half_h, -sens_half_h, sens_half_h, sens_half_h, -sens_half_h])
    pixels_x_sensor = int(np.ceil(sensor_width_mm / resolution))
    pixels_y_sensor = int(np.ceil(sensor_height_mm / resolution))
    pixels_x_sensor = min(pixels_x_sensor, max_pixels)
    pixels_y_sensor = min(pixels_y_sensor, max_pixels)
    px = pixels_x_sensor
    py = pixels_y_sensor

    # --- IFOV calculation (projected pixel edge for each sensor pixel) ---
    ifov_map = None
    min_ifov = max_ifov = 0.0
    if params.get("EnforceIFOV", False):
        def calculate_ifov_map(hw, hh, px, py):
            x_edges = np.linspace(-hw, hw, px + 1)
            y_edges = np.linspace(-hh, hh, py + 1)
            rim_map = np.zeros((px, py))
            for i in range(px):
                for j in range(py):
                    corners = [(x_edges[i], y_edges[j]), (x_edges[i+1], y_edges[j]),
                               (x_edges[i+1], y_edges[j+1]), (x_edges[i], y_edges[j+1])]
                    pts = np.array([project_point(np.array([x, y, 0])) for (x, y) in corners])
                    lens = [np.linalg.norm(pts[k] - pts[(k+1) % 4]) for k in range(4)]
                    rim_map[i, j] = max(lens)
            return rim_map

        ifov_map = calculate_ifov_map(sens_half_w, sens_half_h, px, py)
        max_ifov = float(np.nanmax(ifov_map))
        min_ifov = float(np.nanmin(ifov_map))

    # --- Rectangle corners for overlays ---
    rect_corners = np.array([
        [-W/2, -L/2, 0], [W/2, -L/2, 0], [W/2, L/2, 0], [-W/2, L/2, 0], [-W/2, -L/2, 0]
    ])
    proj_rect_outline = np.array([project_point(p) for p in rect_corners])

    # --- FOV points for world view overlays ---
    fov_points_world = [
        [-C/2, -L/2, 0], [C/2, -L/2, 0], [C/2, L/2, 0], [-C/2, L/2, 0],
        [0, -L/2, 0], [0, L/2, 0], [-C/2, 0, 0], [C/2, 0, 0]
    ]

    # --- Side view 2D camera center/axes ---
    if shift_axis == 'X':
        side_cam_x = Xc
        side_cam_z = H
        optical_axis_2d = np.array([z_cam[0], z_cam[2]])
    ...
    # --- Optimal tilt calculation & water coverage curve ---
    from projection_calculations import calculate_water_coverage_curve, find_optimal_angle_for_coverage
    ...
    # Guarantee arrays never empty
    coverage_angles = np.array(coverage_angles) if isinstance(coverage_angles, list) else coverage_angles
    coverage_values = np.array(coverage_values) if isinstance(coverage_values, list) else coverage_values

    # --- Markers for current user angle and optimal tilt ---
    try:
        idx = np.argmin(np.abs(coverage_angles - float(theta_deg)))
        water_coverage_percent = float(coverage_values[idx]) if len(coverage_values) else 0.0
    except Exception:
        water_coverage_percent = 0.0
    optimal_angle = float(optimal_angle) if optimal_angle is not None else 0.0
    optimal_coverage = float(optimal_coverage) if optimal_coverage is not None else 0.0
    optimal_angle_rad = np.radians(optimal_angle)
    optimal_axis_2d = np.array([np.sin(optimal_angle_rad), -np.cos(optimal_angle_rad)])

    # --- Naive and realistic target pixel estimation ---
    ellipse_width_with_margin = ellipse_half_width * 2 * margin_factor
    ellipse_height_with_margin = ellipse_half_height * 2 * margin_factor
    ...
    pixels_x_target = int(np.ceil((ellipse_max_x - ellipse_min_x) / resolution))
    pixels_y_target = int(np.ceil((ellipse_max_y - ellipse_min_y) / resolution))
    target_sensor_width_mm = (ellipse_max_x - ellipse_min_x)
    target_sensor_height_mm = (ellipse_max_y - ellipse_min_y)
    ...
    # --- Return all fields required by GUI and plots ---
    return {
        # Basic parameters
        'A': A, 'B': B, 'C': C, 'H': H, 'L': L, 'W': W, 'theta_deg': theta_deg, 'shift_axis': shift_axis,
        'Xc': Xc, 'Yc': Yc,
        ...
        'pixels_x_naive': pixels_x_naive,
        'pixels_y_naive': pixels_y_naive,
        'pixels_x_target': pixels_x_target,
        'pixels_y_target': pixels_y_target,
        'sensor_width_mm': sensor_width_mm,
        'sensor_height_mm': sensor_height_mm,
        'target_sensor_width_mm': target_sensor_width_mm,
        'target_sensor_height_mm': target_sensor_height_mm,
        'max_ifov': max_ifov,
        'min_ifov': min_ifov,
        ...
        # Water coverage results
        'water_coverage_percent': water_coverage_percent,

        # Optimal angle data and coverage arrays
        'optimal_angle': optimal_angle,
        'optimal_coverage': optimal_coverage,
        'coverage_angles': coverage_angles if len(coverage_angles) else np.array([0]),
        'coverage_values': coverage_values if len(coverage_values) else np.array([0]),
