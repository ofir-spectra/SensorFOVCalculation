...
def get_plot_data(params, x_range_mm=(-350, 350), y_range_mm=(-350, 350), n_ticks=9, smoothness=2, smoothing_window=1):
    ...
    target_ifov = float(params.get('Resolution', 0.22))  # Or whatever parameter defines your desired IFOV
    max_pixels = int(params.get('MaxSensorRes', 5000))

    # Initial geometric fit—per-tile
    sensor_width_mm = 2 * sensor_half_width
    sensor_height_mm = 2 * sensor_half_height
    pixels_x = max(2, int(np.ceil(sensor_width_mm / target_ifov)))
    pixels_y = max(2, int(np.ceil(sensor_height_mm / target_ifov)))

    if not enforce_ifov:
        # Usual path: just use these and move on
        pixels_x_sensor = pixels_x
        pixels_y_sensor = pixels_y
        max_ifov = target_ifov
    else:
        # Adaptive: try to reach target IFOV by increasing pixel count up to max allowed
        from projection_calculations import compute_max_projected_pixel_size

        # Compute actual IFOV with initial px/py
        ...
        # If IFOV still too high, increase pixels
        if max_ifov > target_ifov:
            factor = max_ifov / target_ifov
            new_pixels_x = min(int(np.ceil(pixels_x * factor)), max_pixels)
            new_pixels_y = min(int(np.ceil(pixels_y * factor)), max_pixels)
            # Recompute IFOV with increased pixels
            max_ifov = compute_max_projected_pixel_size(
                sensor_half_width, sensor_half_height, new_pixels_x, new_pixels_y,
                cam_pos, x_cam, y_cam, z_cam, H
            )
            pixels_x_sensor = new_pixels_x
            pixels_y_sensor = new_pixels_y
        else:
            # IFOV already OK
            pixels_x_sensor = pixels_x
            pixels_y_sensor = pixels_y
    ...
    nx, ny = [int(v) for v in str(cam_setup_str).lower().split("x")]
    deadzone_mm = float(params.get("DeadZone", 0.3))
    pixel_pitch_um = float(params.get("PixelPitch", 2.0))
    deadzone_px = int(round(deadzone_mm * 1000.0 / pixel_pitch_um))

    # px and py must be defined earlier as pixels PER TILE, not total!
    pixels_x_sensor = nx * px + (nx - 1) * deadzone_px
    pixels_y_sensor = ny * py + (ny - 1) * deadzone_px

    sensor_width_mm = pixels_x_sensor * pixel_pitch_um / 1000.0
    sensor_height_mm = pixels_y_sensor * pixel_pitch_um / 1000.0

   # --- Realistic Sensor FOV calculation ---
    # For the PRIMARY sensor (as projected), typically:
    sensor_left  = np.array([-sensor_half_width,  0, 0])
    sensor_right = np.array([ sensor_half_width,  0, 0])
    sensor_top    = np.array([0,  sensor_half_height, 0])
    sensor_bottom = np.array([0, -sensor_half_height, 0])

    # Project these to world:
    def direction_to_world(sensor_xy):
        # vector from cam_pos through (sensor frame) -> direction
        return (sensor_xy[0] * x_cam + sensor_xy[1] * y_cam + H * z_cam)

    # Horizontal FOV (angle between left/right)
    dir_left = direction_to_world([-sensor_half_width, 0])
    dir_right = direction_to_world([sensor_half_width, 0])
    theta_h = np.degrees(np.arccos(
        np.dot(dir_left, dir_right) / (np.linalg.norm(dir_left) * np.linalg.norm(dir_right))
    )) if (np.linalg.norm(dir_left) > 0 and np.linalg.norm(dir_right) > 0) else 0.0

    # Vertical FOV (angle between top/bottom)
    dir_top = direction_to_world([0, sensor_half_height])
    dir_bottom = direction_to_world([0, -sensor_half_height])
    theta_v = np.degrees(np.arccos(
        np.dot(dir_top, dir_bottom) / (np.linalg.norm(dir_top) * np.linalg.norm(dir_bottom))
    )) if (np.linalg.norm(dir_top) > 0 and np.linalg.norm(dir_bottom) > 0) else 0.0

    FOV_H_sensor = theta_h
    FOV_V_sensor = theta_v
