...
def get_plot_data(params, x_range_mm=(-350, 350), y_range_mm=(-350, 350), n_ticks=9, smoothness=2, smoothing_window=1):
    ...
    if not enforce_ifov:
        ...
        if pixels_x > max_pixels or pixels_y > max_pixels:
            print("[IFOV Debug] ABORT: Initial pixel count is too large—settings impossible or need looser IFOV requirement.")
            return {
                'A': np.nan, 'B': np.nan, 'C': np.nan, 'H': np.nan, 'L': np.nan, 'W': np.nan,
                'theta_deg': np.nan, 'shift_axis': None, 'Xc': np.nan, 'Yc': np.nan,
                'rect_corners': np.full((5, 3), np.nan),
                'proj_rect_pts': np.full((10000, 2), np.nan),
                'proj_ellipse_pts': np.full((200, 2), np.nan),
                'proj_rect_outline': np.full((5, 2), np.nan),
                'box_x_mm': np.full(5, np.nan),
                'box_y_mm': np.full(5, np.nan),
                'fov_points_world': [np.full(3, np.nan)] * 8,
                'ellipse_cx': np.nan,
                'ellipse_cy': np.nan,
                'ellipse_half_width': np.nan,
                'ellipse_half_height': np.nan,
                ...
                'optimal_coverage': np.nan,
                'coverage_angles': np.array([]),
                'coverage_values': np.array([]),
                'optics_radius': np.nan,
                'optics_diameter': np.nan,
                'projection_offset': np.nan,
                'camera_center_x': np.nan,
                'camera_center_z': np.nan,
                'sensor_tangent_2d': np.full(2, np.nan),
                'optical_axis_normalized': np.full(2, np.nan),
                'optimal_axis_2d': np.full(2, np.nan),
            }
        converged = False
        for _ in range(max_iter):
            max_proj_size = compute_max_projected_pixel_size(
                sensor_half_width, sensor_half_height, pixels_x, pixels_y,
                cam_pos, x_cam, y_cam, z_cam, H
            )
            delta_ifov = max_proj_size - resolution
            print(f"[IFOV Debug/Pre-check] Pixels: {pixels_x}x{pixels_y}, Max IFOV: {max_proj_size:.6f} mm, Target IFOV: {resolution:.6f} mm, ΔIFOV: {delta_ifov:.6f}")
            if delta_ifov <= min_delta:
                converged = True
                break
            delta_pixels = int(np.ceil((delta_ifov / (pixel_pitch_um * 1e-3)) * 1000))
            if delta_pixels < 1:
                delta_pixels = 1
            pixels_x += delta_pixels
            pixels_y += delta_pixels
            if pixels_x > max_pixels or pixels_y > max_pixels:
                print("IFOV calculation stopped: too many pixels needed!")
                break
        pixels_x_sensor = int(pixels_x)
        pixels_y_sensor = int(pixels_y)
        max_ifov = float(max_proj_size)
        ellipse_width_with_margin = ellipse_half_width * 2 * (1 + margin_percent / 100)
        ellipse_height_with_margin = ellipse_half_height * 2 * (1 + margin_percent / 100)
        pixels_x_naive = round(ellipse_width_with_margin / resolution)
        pixels_y_naive = round(ellipse_height_with_margin / resolution)
