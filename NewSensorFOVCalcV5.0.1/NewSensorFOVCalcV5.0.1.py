...
def parse_camera_setup(cam_setup_str):
    try:
        nx, ny = map(int, cam_setup_str.split('x'))
        return nx, ny
    except Exception:
        return 1, 1  # fallback single sensor

def dead_zone_pixels(deadzone_mm, pixel_pitch_um):
    # Convert dead zone mm to pixels, given the pixel pitch in um
    try:
        return (1000 * deadzone_mm) / pixel_pitch_um
    except Exception:
        return 0
...
class ProjectionApp:
    def __init__(self, root):
        ...
        self.param_keys = [
            "A", "B", "C", "Tilt", "Margin", "Shift", "Resolution", "DeadZone", "PixelPitch"
        ]
        self.entries = {}
    ...
    def update_simulation_results(self, data):
        ...
        results = [
            ...
            ("Sensor Aspect Ratio", data['aspect_ratio_used'], "-"),
            ("Optics Diameter", f"{data['optics_diameter']:.1f}", "mm"),
        ]
    ...
    def setup_params_frame(self):
        ...
        plot_button.grid(row=next_row+2, column=0, columnspan=2, pady=15)
        ...
        if image_path:
            try:
                ...
                self.img_label.grid(row=next_row+3, column=0, columnspan=2, pady=10, sticky="nsew")
                ...
                img_label.grid(row=next_row+3, column=0, columnspan=2, pady=10)
        else:
    ...
    def get_current_parameters(self):
        try:
            return {
                ...
                'DeadZone': float(self.entries['DeadZone'].get()),
                'PixelPitch': float(self.entries['PixelPitch'].get()),
    ...
    def plot_projection(self):
        ...
        deadzone_mm = params.get("DeadZone", 0)
        pixel_pitch_um = params.get("PixelPitch", 2.0)

        plot_data = get_plot_data(params)

        # PATCH: Augment pixels_x/y for multi-camera and dead zone logic (both naive and sensor)
        dz_pix = dead_zone_pixels(deadzone_mm, pixel_pitch_um)
        # naive
        px_naive_ori = plot_data['pixels_x_naive']
        py_naive_ori = plot_data['pixels_y_naive']
        ...
        # realistic sensor
        px_sensor_ori = plot_data['pixels_x_sensor']
        py_sensor_ori = plot_data['pixels_y_sensor']
