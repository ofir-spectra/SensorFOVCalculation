...
class ProjectionApp:
    ...
    def setup_params_frame(self):
        ...
        for i, (label, default) in enumerate(self.param_labels):
            ...
            if self.param_keys[i] == "Tilt":
                tk.Button(param_frame, text="−", font=self.param_font, width=2,
                        command=lambda: self.adjust_param("Tilt", -1)).grid(row=i, column=2)
                tk.Button(param_frame, text="+", font=self.param_font, width=2,
                        command=lambda: self.adjust_param("Tilt", 1)).grid(row=i, column=3)
            if self.param_keys[i] == "Shift":
                tk.Button(param_frame, text="−", font=self.param_font, width=2,
                        command=lambda: self.adjust_param("Shift", -5)).grid(row=i, column=2)
                tk.Button(param_frame, text="+", font=self.param_font, width=2,
                        command=lambda: self.adjust_param("Shift", 5)).grid(row=i, column=3)
            if self.param_keys[i] == "Margin":
                tk.Button(param_frame, text="−", font=self.param_font, width=2,
                        command=lambda: self.adjust_param("Margin", -1)).grid(row=i, column=2)
                tk.Button(param_frame, text="+", font=self.param_font, width=2,
        ...
        if image_path:
            try:
                ...
                self.img_label.grid(row=shift_axis_row+2, column=0, columnspan=2, pady=10, sticky="nsew")
                ...
                img_label.grid(row=shift_axis_row+2, column=0, columnspan=2, pady=10)
        else:
    ...
    def setup_plot_frame(self):
        ...
        plt.ioff()
        self.fig = plt.figure(figsize=(16, 10), dpi=100)  # Larger size
        #gs = gridspec.GridSpec(2, 2, height_ratios=[1.5, 0.8])  # Top row taller
    ...
    def setup_table_frame(self):
        ...
        table_frame.grid(row=2, column=0, columnspan=2, padx=10, pady=10, sticky="ew")
    ...
    def plot_projected_view(self, data):
        ...
        # === Calculate Required FOV (horizontal and vertical, in degrees) using margin-adjusted L and W ===
        tilt_deg = data.get('theta_deg', 0)
        cam_height = data.get('A', 133)  # Rim to water depth (camera height)
        water_length = float(data.get('L', 0))  # margin-adjusted length
        water_width = float(data.get('W', 0))   # margin-adjusted width
        tilt_rad = np.radians(tilt_deg)
        eff_height = cam_height / np.cos(tilt_rad) if np.cos(tilt_rad) != 0 else cam_height
        fov_h_rad = 2 * np.arctan(water_width / (2 * cam_height)) if water_width > 0 else 0
        fov_h_deg = np.degrees(fov_h_rad)
        fov_v_rad = 2 * np.arctan(water_length / (2 * eff_height)) if water_length > 0 else 0
        fov_v_deg = np.degrees(fov_v_rad)
        fov_text = f"REQUIRED FOV (with margin): {fov_h_deg:.1f}° (H) × {fov_v_deg:.1f}° (V)"

        overlay_text = (
            f"REALISTIC SENSOR ({data['aspect_ratio_used']}): {data['pixels_x_sensor']} x {data['pixels_y_sensor']} [px]\n"
            f"NAIVE RESOLUTION: {data['pixels_x_naive']} x {data['pixels_y_naive']} [px]\n"
            f"WATER COVERAGE: {data['water_coverage_percent']:.1f}%\n"
            f"OPTIMAL TILT ANGLE: {data['optimal_angle']:.1f}°\n"
            f"PROJECTION OFFSET: {data['projection_offset']:.1f} mm from optical axis\n"
            f"{fov_text}"
        )

        ax.text(0.02, 0.98, overlay_text, fontsize=GRAPH_OVERLAY_FONTSIZE, color='black', fontweight='bold',
    ...
    def plot_coverage_view(self, data):
        ...
        ax.set_xlim(0, 60)
        ax.set_ylim(5, 90)  # Limited to 5-90% as requested
